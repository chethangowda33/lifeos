...
import {
  Dumbbell, Flame, Scale, Play, ArrowRight,
  Activity, Sparkles, Footprints, HeartPulse, Gauge, Droplets, Route,
} from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div data-testid={DASHBOARD.root} className="max-w-6xl space-y-6 animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2 max-w-xl">
            Where you stand today. <Link to="/progress" className="text-maroon hover:underline">Progress →</Link>
            {" "}has the charts, records and full history.
          </p>
        </div>
      </div>
      ...
      {/* The one way in. The volume chart, recent list, PR shelf and muscle
          bars that used to sit here were smaller copies of what Progress
          already owns — a dashboard reports status, it isn't a second
          analytics page. They're one tap away. */}
...
function computeDerived(workouts) {
  ...
  return { weekCount, weekVolume, streak };
}
