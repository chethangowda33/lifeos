...
- **PWA** — installable + offline app shell DONE (2026-07-14). Remaining: offline **set-logging write-queue** + **background sync** (queue POSTs while offline, flush on reconnect), and real **background push** for train reminders (currently in-app only via `hooks/useTrainReminder.js`).
